...
export default function ImprovedPlayerPropsPage() {
  ...
  const filteredData = useMemo(() => {
    ...
    return dataToFilter.filter((player) => {
      const matchesSearch = player.nfl_players?.player_name
        .toLowerCase()
        .includes(searchQuery.toLowerCase());
      const matchesPosition =
        selectedPositions.length === 0 ||
        selectedPositions.includes(player.nfl_players?.position);
      const matchesPropType =
        selectedTab === "Season Long" ||
        (selectedPropType === "Passing" &&
          (player.passyardsou !== null || player.passtdsnumber !== null)) ||
        (selectedPropType === "Rushing" &&
          (player.rushyardsou !== null || player.rushattempts !== null)) ||
        (selectedPropType === "Receiving" &&
      ...
      <TableRow key={player.playerID}>
        <TableCell className="font-medium">
          ...
          <div className="text-sm text-muted-foreground">
            {player.nfl_players?.position} - {player.nfl_players?.team}
          </div>
        ...
        <TableCell>
          {player.passtdsnumber ? (
            <>
              ...
              <br />U {player.passtdsnumber} {player.passtdsodds}
            </>
          ) : (
            "N/A"
          )}
        ...
        <TableCell>
          {player.passattempts || player.rushattempts || "N/A"}
        </TableCell>
        <TableCell>
          {player.passcompletions || player.receptionsou || "N/A"}
        </TableCell>
      </TableRow>
    ));
  };
            ...
            <CardContent className="space-y-4">
              <div className="flex gap-4">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setSelectedPositions(value.split(",").filter(Boolean))
                  }
                >
                  <SelectTrigger className="w-[180px]">
